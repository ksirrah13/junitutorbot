...
import { processWorlframPods, sendTextResponse } from './worlfram_utils.js';
...
export const doWolfram = async (prompt, thread, parentPromptId) => {
  try {
    ...
    const queryResult = await waApi
      .getFull({ input: prompt, output: 'json', podstate: 'Step-by-step solution' });
    // const queryResult = await waApi
    //   .getSimple(prompt);
    ...
    const podResults = await processWorlframPods(queryResult, thread, worlframClassification);
    if (!podResults) {
